...
def apply_battery_guard(
    ...
    *,
    ...
    if battery_action == "critical":
        print_event(
            f"Batteria {battery}%: atterraggio",
            prefix="ERRORE",
        )
    ...
    def _warn_low_battery(last_at: float) -> float:
        ...
        if now_warn - last_at >= APP_CONFIG.battery_warning_repeat_after_sec:
            print_event(f"Batteria {battery}%: atterra presto", prefix="AVVISO")
            return now_warn
        return last_at
    ...
    if battery_action == "return_home":
        if (
            ...
            if can_return_home:
                rth_active = True
                print_event(
                    f"Batteria {battery}%: rientro alla home",
                    prefix="AVVISO",
                )
                return True, rth_active, last_low_battery_warn_at
            print_event(f"Batteria {battery}%: rientro escluso", prefix="ERRORE")
            print_event("Atterraggio sul posto", prefix="ERRORE")
            vision_loop.clear_autopilot_overlay()
            pilot_commands.disable_autonomy()
            try:
                controller.land()
...
def apply_desync_guard(subsystems: Subsystems, ground_since: Optional[float]) -> Optional[float]:
    ...
    if landed_confirmed:
        print_event("Drone a terra: stato riallineato", prefix="AVVISO")
        if pilot_commands.is_autonomy_enabled():
            vision_loop.clear_autopilot_overlay()
            pilot_commands.disable_autonomy()
