...
_LOG_GLYPH_SHAPES = {"·": "info", "•": "info", "!": "avviso", "×": "allerta", "✓": "esito"}
...
_LEGEND_ROW_H = 28
_LEGEND_PAD_X = _CARD_PAD_X
_LEGEND_TITLE_Y = 21
_LEGEND_TITLE_GAP = 16
_LEGEND_PAD_Y = 15
_LEGEND_COL_GAP = 14
...
def _paint_log_glyph(d, forma, cx, cy, r, color) -> None:
    if forma == "avviso":
        alto = r * 1.15
        d.polygon(
            [(cx, cy - alto), (cx + alto, cy + alto * 0.78), (cx - alto, cy + alto * 0.78)],
            fill=color,
        )
        return
    if forma == "allerta":
    ...
    if forma == "esito":
        w = max(1, round(r * 0.5))
        d.line((cx - r, cy + r * 0.1, cx - r * 0.2, cy + r * 0.8), fill=color, width=w)
        d.line((cx - r * 0.2, cy + r * 0.8, cx + r, cy - r * 0.8), fill=color, width=w)
        return
    mezzo = r * 0.55
    d.ellipse((cx - mezzo, cy - mezzo, cx + mezzo, cy + mezzo), fill=color)


def _draw_log_glyph(img, forma, cx, cy, r, color) -> None:
    reach = r * 1.4
    paint_supersampled(
        img, (cx - reach, cy - reach, cx + reach, cy + reach), (cx, cy),
        lambda d, ax, ay, k, forma=forma: _paint_log_glyph(d, forma, ax, ay, r * k, color),
    )


def _visible_log_pieces(lines, max_chars: int, max_visible: int) -> list[tuple[str, bool, str]]:
    if max_visible <= 0:
        return []
    visible: deque[tuple[str, bool, str]] = deque()
    for line in reversed(lines):
        line = str(line)
        pezzi = _layout_log_line(line, max_chars)
        if len(visible) + len(pezzi) > max_visible:
            if visible:
                break
            pezzi = pezzi[:max_visible]
        visible.extendleft(
            (piece, indented, line) for piece, indented in reversed(pezzi)
        )
...
def _draw_lines(img, draw, lines, *, left: int, right: int, top: int, bottom: int,
    ...
    y = top
    raggio = char_w * 0.42
    for piece, indented, line in _visible_log_pieces(lines, max_chars, max_visible):
        x = left + (indent_px if indented else 0)
        text_color = _TEXT if line_color is None else line_color(line)
        forma = "" if indented else _LOG_GLYPH_SHAPES.get(_severity_glyph(piece), "")
        cy = y + _LOG_LINE_H / 2
        if forma:
            piece = piece[:_GLYPH_COLUMN] + " " + piece[_GLYPH_COLUMN + 1:]
        draw.text((x, cy), piece, font=font, fill=text_color, anchor="lm")
        if forma:
            _draw_log_glyph(
                img, forma, x + (_GLYPH_COLUMN + 0.5) * char_w, cy, raggio, text_color,
            )
        y += _LOG_LINE_H
...
def text_panel(config, title: str, lines, height: int, *, engaged: bool,
    ...
    draw.text((x0 + _LOG_TEXT_PAD, y0 + _LOG_TITLE_Y), title,
              font=fonts.sans_bold(_FONT_CARD_TITLE), fill=_WHITE, anchor="lm")
    ...
    if not engaged:
        ...
        return _finalize(img)
    _draw_lines(
        img, draw, lines, left=x0 + _LOG_TEXT_PAD, right=x1 - _LOG_TEXT_PAD,
...
def _numero(value, decimals: int) -> str:
    return f"{value:.{decimals}f}".replace(".", ",")


def _measure_parts(value, decimals: int, unit: str) -> tuple[str, str]:
    if value is None:
        return "--", ""
    return _numero(value, decimals), unit
...
def _unit_column_width(draw, axes, font) -> int:
    unita = {unit for _n, _e, _t, _ok, unit, _d in axes if unit}
    if not unita:
        return 0
    return int(max(draw.textlength(u, font=font) for u in unita))


def _draw_kv_line(draw, x0: int, x1: int, y_center: int, label: str, value,
                  unit_w: int = 0) -> None:
    font = fonts.sans(_FONT_KV)
    draw.text((x0, y_center), label, font=font, fill=_LABEL, anchor="lm")
    numero, unita = value if isinstance(value, tuple) else (value, "")
    if unita and unit_w:
        stacco = _DEGREE_GAP if unita == DEGREE_UNIT else _UNIT_GAP
        draw.text((x1 - unit_w, y_center), unita, font=font, fill=_WHITE, anchor="lm")
        draw.text((x1 - unit_w - stacco, y_center), numero, font=font, fill=_WHITE,
                  anchor="rm")
        return
    draw.text((x1, y_center), numero, font=font, fill=_WHITE, anchor="rm")
...
def _draw_tolerance_card(img, x0: int, y0: int, width: int, axes) -> None:
    ...
    unit_w = _unit_column_width(draw, axes, fonts.sans(_FONT_KV))
    ...
    for index, (name, err, tol, ok, unit, decimals) in enumerate(axes):
        ...
        _draw_kv_line(
            ...
            _measure_parts(err, decimals, unit), unit_w,
        )
        _draw_kv_line(
            draw, inner_x0, inner_x1, top + _AXIS_LINE_Y[1],
            TOLERANCE_THRESHOLD_LABEL, _measure_parts(tol, decimals, unit), unit_w,
        )
...
def _draw_timer_box(img, x0: int, y0: int, width: int, mission_map) -> None:
    ...
    value = "--" if remaining is None else f"{_numero(remaining, 1)} s"
    draw.text(
...
def _map_legend_h() -> int:
    ...
    return _LEGEND_TITLE_Y + _LEGEND_TITLE_GAP + _LEGEND_ROW_H * rows + _LEGEND_PAD_Y
...
def _paint_legend_swatch(draw, cx: float, cy: float, k: int, kind: str, fill, border) -> None:
    ...
    if kind == "dot":
        raggio = half * 0.88
        draw.ellipse((cx - raggio, cy - raggio, cx + raggio, cy + raggio), fill=fill)
        return
...
def _draw_map_legend(img, x0: int, y0: int, width: int) -> None:
    ...
    draw = ImageDraw.Draw(img)
    draw.text(
        (left_x, y0 + _LEGEND_TITLE_Y), LEGEND_TITLE,
        font=fonts.sans_bold(_FONT_CARD_TITLE), fill=_WHITE, anchor="lm",
    )

    top = y0 + _LEGEND_TITLE_Y + _LEGEND_TITLE_GAP
