...
def _pi_wifi(draw, cx, cy, s, color):
    ...
    for rad, a0, a1 in ((4.6, 200, 340), (8.2, 205, 335)):
        r = rad * s
...
def _pi_drone(draw, cx, cy, s, color):
    w = _icon_stroke(s)
    draw.line((cx - 5.4 * s, cy - 4.4 * s, cx + 5.4 * s, cy + 4.4 * s), fill=color, width=w)
    draw.line((cx + 5.4 * s, cy - 4.4 * s, cx - 5.4 * s, cy + 4.4 * s), fill=color, width=w)
    for dx, dy in ((-5.4, -4.4), (5.4, -4.4), (-5.4, 4.4), (5.4, 4.4)):
        ex, ey, r = cx + dx * s, cy + dy * s, 2.6 * s
...
_PANEL_ICONS = (_pi_wifi, _pi_gamepad, _pi_drone, _pi_eye, _pi_pin)
...
def _status_rows(connected, joystick, flying, detection_enabled, autonomy_enabled):
    stati = (
        ...
        bool(detection_enabled), bool(autonomy_enabled),
    )
    return list(zip(_PANEL_LABELS, stati, _PANEL_ICONS))


def _watch_rows(connected):
    return list(zip(_WATCH_LABELS, (bool(connected),), _WATCH_ICONS))
...
def draw_cached_detections(frame, cached):
    for entry in cached:
        ...
        for det in entry["detections"]:
            ...
            nome = det.get("display_label") or det.get("label") or ""
            label = f"{nome} {round(float(det['confidence']) * 100)}%"
