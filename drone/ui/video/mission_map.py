...
from drone.ui.shapes import draw_centered_label
...
def _draw_tags(draw, tags, to_px, seen_ids=()) -> None:
    ...
    for tag in tags:
        ...
        if tag.tag_id in seen_ids:
            ...
            draw_centered_label(draw, cx, cy, str(tag.tag_id), font_seen, _MAP_BG)
            continue
        draw.rectangle(
            (cx - half, cy - half, cx + half, cy + half),
        ...
        draw_centered_label(draw, cx, cy, str(tag.tag_id), font, color)
